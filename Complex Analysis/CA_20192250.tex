...
\definecolor{sumcolor}{RGB}{84, 148, 171}
...
\begin{document}
	...
	\begin{center}
		...
		\filldraw[black] (-1,2) circle (1pt) node[anchor=south west] {$z$};
		
		\draw[->] (0,0) -- (1,1) node[midway, above left] {$r_1$};
		\draw[->] (0,0) -- (.5,1.5) node[midway, above left] {$r_2$};
		\draw[->] (0,0) -- (-1,2) node[midway, left] {$r=r_1+r_2$};
		\draw[->, thick, blue] (.3,0) arc (0:45:.3) node[midway, right, blue] {$\theta_1$};
		\draw[->, thick, blue] (.7,0) arc (0:71:.7) node[midway, right, blue] {$\theta_2$};
		\draw[->, thick, red] (1,0) arc (0:116:1) node[midway, right, red] {$\theta=\theta_1+\theta_2$};
		\end{tikzpicture}
	\end{center}
	
	\section{De Movire's Formula and $n$th roots}
	...
	\begin{remark}[Conformality]
			...
			\begin{minipage}{.45\textwidth}
				...
				\draw[line width=.5mm, red] (0,0) -- (1.4,1.4) node[midway, above left] {};
				\draw[->, thick, red] (.5,0) arc (0:45:.5) node[midway, right, red] {$\theta$};
