...
\begin{document}
	...
	Finding $a$ and $b$ that satisfy $rN=a^2-b^2$ is equivalent to finding $a$ and $b$ satisfying \begin{enumerate}[(i)]
		\item $a^2\equiv b^2\pmod{N}$,
		\item $\gcd\of{N,a-b}\in\of{1,N}$.
	\end{enumerate}
	...
	\begin{proof}
		\begin{align*}
		a_i^{(p)}=a_j^{(p)}&\iff a_i\mod p=a_j\mod p\\
		&\iff a_i\equiv a_j\pmod{p}\\
		&\iff a_i^2\equiv a_j^2\pmod{p}\\
		&\iff a_i^2+c\equiv a_j^2+c\pmod{p}\\
		&\iff \of{a_i^2+c\mod pq}\mod p=\of{a_j^2+c\mod pq}\mod p\\
		&\iff a_{i+1}\mod p=a_{j+1}\mod p\\
		&\iff a_{i+1}^{(p)}= a_{j+1}^{(p)}.
		\end{align*}
	\end{proof}
	...
	\begin{algorithm}[H]
		...
		\KwData{$N=pq$ ($p,q$: distinct odd primes), $c$: constant.}
		\KwResult{$p$ and $q$.}
		\vspace{4pt}
		\vspace{4pt}
		$c\uniform\Z_N$\;
		\vspace{4pt}
		\Fn{$f\of{x}$}{
			\Return $\of{x^2 + c}\mod N$\;
		}
		\vspace{4pt}
		$(a,b)\gets\of{2,2}$\;
		\vspace{4pt}
		\While{\texttt{True}}{
			$\of{a,b}\gets\of{f\of{a},\of{f\circ f}\of{b}}$\;
			$d\gets\gcd\of{N,a-b}$\;
			\If{$1<d<N$}{
				\Return $\of{d, N/d}$\;
			}
	...
	\begin{center}
		...
		\node[block, right=2.5cm of C] (D) {$\mu$\nodepart{two}$f^\mu(x_0)$};
		\node[block, right=of D] (E) {$\mu + 1$\nodepart{two}$f^{\mu + 1}(x_0)$};
		\node[block, right=of E] (F) {$\cdots$\nodepart{two}$\cdots$};
		\node[block, right=of F] (G) {$\nu$\nodepart{two}$f^{\nu}(x_0)$};
	...
	\begin{algorithm}[H]
		...
		$m \leftarrow \lceil\sqrt{n}\rceil+1$ \tcp*{$m>\sqrt{n}$}
	...
	\end{algorithm}
	\vspace{8pt}
	Consider cyclic group $G=\langle g\rangle$ of order $n$. We want to find $k$ such that $g^k=h$ for some $h\in G$. Clearly, $k\in\set{0,1,\cdots, n-1}=\Z_n$. Define a number $m:=\ceil*{\sqrt{n}}+1$. By division theorem, $\exists!q,r\in\Z_{\geq 0}$ such that \[
	...
	\end{align*} Since $BS\cap GS\neq\emptyset$ clearly, we can find $i,j\in\set{0,1,\cdots,m-1}$ such that $g^i=h\of{g^{-m}}^j$. Hence we obtain  the discrete logarithm \[
	k=mj+i.
	\]
	...
	\begin{itemize}
		...
		\of{1_G,0,0}=\of{g^0h^0, 0, 0} &: j=0,\\
		\of{g^0h^0, 0, 0} &: j=0,\\
		1_G=\of{g^0h^0, 0, 0} &: j=0,\\
		1_G=\of{g^0h^0, 0, 0} &: j=0.
		\end{cases}
		\]
		\item[] (Step III)
	...
	The Pollard's rho method is an algorithm designed to solve the discrete logarithm problem (DLP), which is defined as follows: given a group G with generator g and an element h in G, find an integer x such that $g^x = h$ (mod p), where p is a prime number.
	
	The algorithm is based on the idea of finding a collision in a sequence of group elements and their corresponding exponents. It employs a pseudorandom function $F(x)$ to generate the sequence.
	
	The algorithm's main steps are as follows:
	
	1. Define a pseudorandom function $F(x)$ that maps elements of the group G to itself. The function can be partitioned into three equal-sized domains, each with its own transformation rule:
	
	\[
	F(x) = 
	\begin{cases}
	(x, a + 1, b) & \text{if } 0 \leq x < \frac{1}{3}p \\
	(x^2, 2a, 2b) & \text{if } \frac{1}{3}p \leq x < \frac{2}{3}p \\
	(x \cdot h, a, b + 1) & \text{otherwise}
	\end{cases}
	\]
	
	2. Initialize two sequences of group elements, $x_1$ and $x_2$, both starting at the identity element $1$. The sequences are generated by applying the function $F(x)$ iteratively.
	
	3. Search for a collision between the two sequences, i.e., find distinct indices $i$ and $j$ such that $x_i = x_j$. The sequences are traversed using the "tortoise and hare" approach, with $x_1$ moving one step at a time and $x_2$ moving two steps at a time.
	
	4. Once a collision is found, compute the difference between the exponents of the colliding elements, $r = b_i - b_j \pmod{p}$. Calculate the inverse of $r$, denoted as $r^{-1} \pmod{p}$.
	
	5. Calculate the discrete logarithm x as $x = (a_j - a_i) \cdot r^{-1} \pmod{p}$.
	
	The complexity of the algorithm is $O(\sqrt{p})$, as it involves generating and searching through a sequence of group elements until a collision is found.
	
	Here is the LaTeX code for the equations presented above:
	
	\[
	\text{Step 1: Define the pseudorandom function } F(x) \text{:}
	\]
	\[
	F(x) = 
	\begin{cases}
	...
	\[
	\text{Step 4: Calculate the inverse of } r \text{:}
	\]
	\[
	r^{-1} = r^{-1} \pmod{p}
	\]
	...
	\[
	\text{Step 5: Calculate the discrete logarithm x:}
	\]
	\[
	x = (a_j - a_i) \cdot r
	^{-1} \pmod{p}
	...
	You can paste this code into a LaTeX editor or a platform that supports LaTeX, such as Overleaf or Jupyter Notebooks, to render the equations properly.
