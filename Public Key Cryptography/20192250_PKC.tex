...
int main() {
	...
	\begin{remark}[Notation]
		The 1-dimensional subspace S of V can be defined as follows: \[
		S=\set{\of{\lambda a,\lambda b}:\lambda\in K}\subseteq V
		\] for some $(a, b)\setminus\set{(0,0)}$. Then $S$ is denoted as $S=(a:b)$. That is, \[
		\mathbb{P}^1(K)=\set{(a:b):a,b\in K\setminus\set{0}}
		\]
	\end{remark}
	\vspace{4pt}
	\begin{example}
		Consider $\mathbb{P}^1(\F_3)=\mathbb{P}^1(\set{0,1,2})$. Then \[
		\begin{cases}
		\mathcolorbox{-red}{(0:1)}\\
		\mathcolorbox{-red}{(0:2)}\\
		\mathcolorbox{-blue}{(1:0)}\\
		\mathcolorbox{-green}{(1:1)}\\
		\mathcolorbox{green}{(1:2)}\\
		\mathcolorbox{-blue}{(2:0)}\\
		\mathcolorbox{green}{(2:1)}\\
		\mathcolorbox{-green}{(2:2)}
		...
		\begin{center}
			...
			every node/.style = {shape=rectangle, rounded corners,
				draw, align=center}]
			\node {$\mathbb{P}^1(\F_3)$}
			child { node {$(0:1)$\\ $(1:1)$\\ $(2:1)$} }
			child { node {$\underbrace{(1:0)}_{\text{point at infinity}}$} };
			\end{tikzpicture}
		\end{center}
		\iffalse
		\begin{center}
			...
			%add straight line
			\draw[line width=.25mm, red] (-1.5,-1.5) -- (2.5,2.5) node[above] {$(1:1)$};
			\draw[line width=.25mm, red] (-3,-1.5) -- (4,2) node[above] {$(2:1)$};
			\draw[line width=.25mm, red] (0,-1.5) -- (0,2.5) node[above right] {$(0:1)$};
			\draw[line width=.25mm, red] (-5,0) -- (5,0) node[above] {$(1:0)$};
			%draw point
			\filldraw[blue] (0,1) circle (1.5pt) node[] {};
			\filldraw[blue] (1,1) circle (1.5pt) node[] {};
			\filldraw[blue] (2,1) circle (1.5pt) node[] {};
			\filldraw[blue] (4.5,0) circle (1.5pt) node[] {};
			
			%draw line
			\draw[line width=.25mm, blue] (0,1) -- (1,1) node[] {};
			\draw[line width=.25mm, blue] (1,1) -- (2,1) node[] {};
			\draw[line width=.25mm, blue] (2,1) .. controls (4,1) and (4,1) .. (4.5,0) node[] {};
	...
	\begin{tcolorbox}[colback=white,colframe=defcolor,arc=5pt,title={\color{white}\bf Projective Plane}]
		\begin{definition}
			Let $K$ be a field. \begin{enumerate}[(1)]
				\item In the $3$-dimensional vector space $V=K^3$ defined over the field $K$, a $1$-dimensional subspace is referred to as a \textbf{projective plane}.
				\item The set $\mathbb{P}^2(K)$ of all projective planes is called a \textit{$2$-dimensional projective space}. \[
				\mathbb{P}^2(K)=\set{S\subseteq V=K^3:\dim{S}=1}
				\]
				\item An element of a projective space is referred to as a \textit{point}.
			\end{enumerate}
		\end{definition}
	...
	\begin{remark}[Notation]
		The 1-dimensional subspace $S$ of $V$ can be defined as follows: \[
		S=\set{\of{\lambda a,\lambda b, \lambda c}:\lambda\in K}\subseteq V
		\] for some $(a, b, c)\setminus\set{(0,0,0)}$. Then $S$ is denoted as $S=(a:b:c)$. That is, \[
		\mathbb{P}^2(K)=\set{(a:b:c):a,b,c\in K\setminus\set{0}}.
		\]
	\end{remark}
	\vspace{4pt}
	\begin{example}
		Consider $\mathbb{P}^2(\F_3)=\mathbb{P}^2(\set{0,1,2})$. Then
		\begin{center}
			\begin{tikzpicture}[sibling distance=10em,
			every node/.style = {shape=rectangle, rounded corners,
				draw, align=center}]
			\node {$\mathbb{P}^2(\F_3)$}
			child { node {$(*:*:1)$\\ $3\times3=9$ items} }
			child { node {$(*:*:0)$} 
				child { node {$(*:1:0)$\\ $3$ items} }
				child { node {$(*:0:0)$\\ $1$ item} }
			};
			\end{tikzpicture}
		\end{center}
	\end{example}
	...
	\begin{example}
		...
		\begin{center}
			...
			\node {$\mathbb{P}^2(\F_7)$}
			child { node {$(*:*:1)$}
				child { node {$(0:0:1)$\\ $\vdots$\\ $(6:6:1)$} }
			}
			child { node {$(*:*:0)$}
				child { node {$(0:1:0)$\\ $\vdots$\\ $(6:1:0)$} }
				child { node {$(1:0:0)$} }
			};
			\end{tikzpicture}
		\end{center}
	...
	\begin{tcolorbox}[colback=white,colframe=defcolor,arc=5pt,title={\color{white}\bf Line}]
		\begin{definition}
			On the projective plane $\mathbb{P}^2(K)$, the set of points satisfying the following homogeneous linear equation is called a \textbf{line}: \[
			\alpha X+\beta Y+\gamma Z=0,\quad \alpha,\beta,\gamma\in K\setminus\set{0}.
			\] Therefore, the line $L(\alpha,\beta,\gamma)$ can be expressed as follows: \[
			L(\alpha,\beta,\gamma):=\set{(a:b:c)\in\mathbb{P}^2(K):\alpha a+\beta b+\gamma c=0}.
			\]
		\end{definition}
	\end{tcolorbox}
	\vspace{4pt}
	\begin{example}
		Find the points on the following line defined in $\mathbb{P}^2(\F_5)$. \[
		...
		\begin{proof}[\sol]
			...
			\end{center} \begin{enumerate}
				\item[$(f_1)$] $2X+3Y+Z=0$; \[
				\begin{cases}
				2X+3Y=0 &: Z=0\quad\cdots\cdots\text{(i)}\\
				2X+3Y+1=0 &: Z=1\quad\cdots\cdots\text{(ii)}
				\end{cases}
				\] \begin{multicols}{2}
					(i)
					\begin{align*}
				...
				\] \begin{multicols}{2}
					...
					\end{align*}
				\end{multicols} Therefore, \[
				(3:1:0)\quad\text{and}\quad\begin{cases}
				(0:2:1)\\
				(1:4:1)\\
				(2:1:1)\\
				(3:3:1)\\
				...
				\end{center}
				Therefore, \[
				(1:0:0)\quad\text{and}\quad \begin{cases}
				(0:1:0)\\
				(1:1:0)\\
				(2:1:0)\\
				(3:1:0)\\
	...
	% Draw the line through P and Q and the point -R
